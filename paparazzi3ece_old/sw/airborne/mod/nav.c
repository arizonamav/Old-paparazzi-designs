/*
 * $Id: nav.c,v 1.36 2006/10/17 15:07:14 hecto Exp $
 *  
...
void course_pid_run( void ) {
  ...
  float roll_from_err  = course_pgain * speed_depend_nav*(in_circle?1:Min(1, dist2_to_wp/100.0f)) * err;
...
float gaz_of_climb=CLIMB_GAZ_OF_CLIMB;
...
climb_pid_run ( void ) {
  ...
  float cmd_climb=climb_pgain * err + climb_igain * climb_sum_err;
  float controlled_gaz = climb_level_gaz + gaz_of_climb*cmd_climb;

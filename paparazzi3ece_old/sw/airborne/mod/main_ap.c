/*
 * $Id: main_ap.c,v 1.67 2006/10/17 16:57:47 poine Exp $
 *  
 ...
 *  \brief Compute desired course and altitude
 */
...
void guidance_task(void)
{
  ...
  if (	   pprz_mode == PPRZ_MODE_AUTO2 
	|| pprz_mode == PPRZ_MODE_HOME
	|| pprz_mode == PPRZ_MODE_GPS_OUT_OF_ORDER) {
    if (lateral_mode >=LATERAL_MODE_COURSE) course_pid_run(); /* aka compute nav_desired_roll */
    desired_roll = nav_desired_roll;
    if (vertical_mode >= VERTICAL_MODE_AUTO_CLIMB) climb_pid_run();
    if (vertical_mode == VERTICAL_MODE_AUTO_GAZ) desired_gaz = nav_desired_gaz;
    desired_pitch = nav_pitch;
    if (kill_throttle || (!estimator_flight_time && !launch)) desired_gaz = 0;
  }
 ...
 */
void controllability_task(void)
{

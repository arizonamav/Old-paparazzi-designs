/*
 * $Id: aerocomm.h,v 1.0 2008/1/19 16:53:16 rkrash Exp $
 *  
...
#define AerocommTransportCheckFreeSpace(x) (AerocommLink(CheckFreeSpace(x)) && aerocomm_confirmation_received)
...
#define AerocommTransportHeader(_len) { \
  aerocomm_cs = 0; \
  AerocommTransportPutTXHeader(); \
  AerocommTransportPut1Byte(_len+2); \
  AerocommTransportPut1Byte(AEROCOMM_COUNT); \
  AerocommTransportPut1Byte(AEROCOMM_RETRIES); \
  AerocommTransportPutUint8(0x00); \
  AerocommTransportPutUint8(GROUND_STATION_ADDR >> 8); \
  AerocommTransportPutUint8(GROUND_STATION_ADDR & 0xff); \
}

#define AerocommTransportTrailer() { \
  aerocomm_confirmation_received=FALSE;\
  AerocommTransportSendMessage(); \
  uint32_t j=10000;\
  while (j && !aerocomm_confirmation_received) {\
	j--;\
  	ReadAerocommBuffer();\

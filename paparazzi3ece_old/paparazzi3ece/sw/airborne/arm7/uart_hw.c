...
static void uart0_init_param( uint16_t baud, uint8_t mode, uint8_t fmode) {
  ...
  VICVectCntl6 = VIC_ENABLE | VIC_UART0;
  VICVectAddr6 = (uint32_t)uart0_ISR;    // address of the ISR
...
static void uart1_init_param( uint16_t baud, uint8_t mode, uint8_t fmode) {
  ...
  VICVectCntl7 = VIC_ENABLE | VIC_UART1;
  VICVectAddr7 = (uint32_t)uart1_ISR;    // address of the ISR

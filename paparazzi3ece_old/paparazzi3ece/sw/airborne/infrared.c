...
int16_t adc_roll_neutral;
int16_t adc_pitch_neutral;
...
void ir_init(void) {
  ...
  adc_roll_neutral=IR_ADC_ROLL_NEUTRAL;
  adc_pitch_neutral=IR_ADC_PITCH_NEUTRAL;
...
void ir_update(void) {
  ...
  ir_roll -= adc_roll_neutral;
  ir_pitch -= adc_pitch_neutral;
#endif /* !SITL && !HITL */
...
#warning "360 deg estimation"
  if (ir_360) { /* 360° estimation */
    ...
    estimator_phi  = atan2(tmp_ir_roll, tmp_ir_top) - RadOfDeg(ir_roll_neutral);
    estimator_phi = correct_angle(estimator_phi, ir_estimated_phi_pi_4, ir_estimated_phi_minus_pi_4);

    estimator_theta  = atan2(tmp_ir_pitch, tmp_ir_top) - RadOfDeg(ir_pitch_neutral);
...
#endif /* IR_360 */
  {
    ...
    estimator_phi  = c * ir_roll - RadOfDeg(ir_roll_neutral);
    estimator_theta = c * ir_pitch - RadOfDeg(ir_pitch_neutral);

/*
 * $Id: main_ap.c,v 1.77 2007/01/27 00:38:29 poine Exp $
 *  
...
/*#ifdef TRANSITION
#warning 'transition mode is used'
    h_ctl_pitch_setpoint+=h_ctl_pitch_state_setpoint;
#endif*/
...
#endif
#ifdef CAM_PITCH
    ap_state->commands[COMMAND_CAM_PITCH] = fbw_state->channels[RADIO_CAM_PITCH];
#endif		
...
{
//Transition mode is used only for mini-vertigo. It allows for smooth transition of the vehicle from low aoa to high aoa and vice versa.
#ifdef TRANSITION
	switch (transition_mode) {
		case 1:
		h_ctl_pitch_state_setpoint=0;
		h_ctl_lo_throttle_roll_rate_pgain=h_ctl_mid_aoa_roll_rate_pgain;
		h_ctl_hi_throttle_roll_rate_pgain=h_ctl_mid_aoa_roll_rate_pgain;		
		h_ctl_lo_throttle_pitch_rate_pgain=h_ctl_mid_aoa_pitch_rate_pgain;
		h_ctl_hi_throttle_pitch_rate_pgain=h_ctl_mid_aoa_pitch_rate_pgain;
		h_ctl_lo_throttle_yaw_rate_pgain=h_ctl_mid_aoa_yaw_rate_pgain;
		h_ctl_hi_throttle_yaw_rate_pgain=h_ctl_mid_aoa_yaw_rate_pgain;
		h_ctl_elevator_of_roll=h_ctl_mid_aoa_elevator_of_roll;
		break;
		case 2:
		h_ctl_pitch_state_setpoint=RadOfDeg(35);
		h_ctl_lo_throttle_roll_rate_pgain=h_ctl_hi_aoa_roll_rate_pgain;
		h_ctl_hi_throttle_roll_rate_pgain=h_ctl_hi_aoa_roll_rate_pgain;		
		h_ctl_lo_throttle_pitch_rate_pgain=h_ctl_hi_aoa_pitch_rate_pgain;
		h_ctl_hi_throttle_pitch_rate_pgain=h_ctl_hi_aoa_pitch_rate_pgain;
		h_ctl_lo_throttle_yaw_rate_pgain=h_ctl_hi_aoa_yaw_rate_pgain;
		h_ctl_hi_throttle_yaw_rate_pgain=h_ctl_hi_aoa_yaw_rate_pgain;
		h_ctl_elevator_of_roll=h_ctl_hi_aoa_elevator_of_roll;
		break;
		case 3:
		h_ctl_pitch_state_setpoint=RadOfDeg(-15);
		h_ctl_lo_throttle_roll_rate_pgain=h_ctl_lo_aoa_roll_rate_pgain;
		h_ctl_hi_throttle_roll_rate_pgain=h_ctl_lo_aoa_roll_rate_pgain;		
		h_ctl_lo_throttle_pitch_rate_pgain=h_ctl_lo_aoa_pitch_rate_pgain;
		h_ctl_hi_throttle_pitch_rate_pgain=h_ctl_lo_aoa_pitch_rate_pgain;
		h_ctl_lo_throttle_yaw_rate_pgain=h_ctl_lo_aoa_yaw_rate_pgain;
		h_ctl_hi_throttle_yaw_rate_pgain=h_ctl_lo_aoa_yaw_rate_pgain;
		h_ctl_elevator_of_roll=h_ctl_lo_aoa_elevator_of_roll;
		break;
}
#endif
...
float alt=GROUND_ALT+(float)(fbw_state->channels[RADIO_THROTTLE])/9600.0f*3.0f;
...
void periodic_task_ap( void ) {
  ...
  case 4:
    enose_periodic();
    DOWNLINK_SEND_ENOSE_STATUS(&enose_sensor_data[0], &enose_sensor_data[1], &enose_sensor_data[2], &FCG1, &FCG2, &FCG3, & enose_heating_setting);
    break;
#endif 
  ...
  case 5:
	  vn100_periodic();
	  DOWNLINK_SEND_VN100_STATUS(&Roll, &Pitch, &Yaw);
	  break;
#endif
   /*  default: */
  }
...
Not sure why spi_init(); is not needed to utilize the spi interface

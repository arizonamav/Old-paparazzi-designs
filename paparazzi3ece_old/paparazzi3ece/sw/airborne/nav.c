/*
 * $Id: nav.c,v 1.50 2007/01/31 15:52:54 hecto Exp $
 *  
...
float nav_altitude = SECURITY_ALT;
...
//  if (fabs(nav_altitude - last_nav_altitude) > 1.) //Roman 
//  {
...
void open_bay(void)
{
	ap_state->commands[COMMAND_CAM_ROLL]=-MAX_PPRZ;
}
...
/** void bomb(void)
 *\brief drop a sensor by commanding a camera servo to extreme position
**/

void bomb(uint8_t wp)
{
  horizontal_mode = HORIZONTAL_MODE_WAYPOINT;
  float v2_x2=waypoints[wp].x; 
  float v2_y2=waypoints[wp].y;
  float v1_x2=cos(estimator_hspeed_dir);
  float v1_y2=sin(estimator_hspeed_dir);
  fly_to_xy(v2_x2, v2_y2);

  //compute minimum distance to wp with current course
  float v1v2=(v1_x2*(v2_x2-estimator_x)+v1_y2*(v2_y2-estimator_y));
  //float x_min2=dist2_to_wp-v1v2; // mininal distance to the target for the current course
  float X0=(sqrt(estimator_z_dot*estimator_z_dot+2*G*(estimator_z-ground_alt))-estimator_z_dot)/G*estimator_hspeed_mod;
  if (X0*X0<=v1v2) {
	open_bay(); // drop the payload
	empty_bay=60;
  }
}
#endif
